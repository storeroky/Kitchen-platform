...
const PROVIDER = process.env.AI_IMAGE_PROVIDER || 'openai' // openai | stability
...
app.post('/api/generate-design', async (req, res) => {
  ...
  try {
    let imageUrl
    if (referenceImage) {
      // وضع "تحويل صورة SketchUp": نرسل الصورة الأصلية + الوصف ليحافظ الذكاء الاصطناعي على نفس التكوين
      imageUrl =
        PROVIDER === 'stability'
      ...
      // وضع "توليد من الصفر"
      imageUrl = PROVIDER === 'stability' ? await generateWithStability(prompt) : await generateWithOpenAI(prompt)
    }
    res.json({ imageUrl })
  } catch (err) {
